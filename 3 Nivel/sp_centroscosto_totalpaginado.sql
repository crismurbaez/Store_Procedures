-- FUNCTION: public.sp_centroscosto_totalpaginado(refcursor, integer, integer, character varying, character varying, character varying, character varying, character varying, smallint)
-- Calcular total de páginas para paginación de centros de costo

-- DROP FUNCTION IF EXISTS public.sp_centroscosto_totalpaginado(refcursor, integer, integer, character varying, character varying, character varying, character varying, character varying, smallint);

CREATE OR REPLACE FUNCTION public.sp_centroscosto_totalpaginado(
...
BEGIN
    -- DEBUG
    IF p_debug = 1 THEN
        RAISE NOTICE 'TOTALPAGINADO → pagina=%, decuantos=%, centrocostoid=%, nombre=%, departamento=%, lugarpago=%, empresa=%',
...
ALTER FUNCTION public.sp_centroscosto_totalpaginado(refcursor, integer, integer, character varying, character varying, character varying, character varying, character varying, smallint)
    OWNER TO postgres;

COMMENT ON FUNCTION public.sp_centroscosto_totalpaginado(refcursor, integer, integer, character varying, character varying, character varying, character varying, character varying, smallint)
    IS 'Calcular total de páginas para paginación de centros de costo';
